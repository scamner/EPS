    ...
    public class RunWorkflowItem
    {
        public Utilities.ItemRunResult RunItem(int EmpID, RunPayload RunPayload)
        {
            ...
            try
            {
                ...
                return new Utilities.ItemRunResult { ResultID = 2, ResultText = String.Format("{0} {1} was un-hidden from the Exchange address lists.", emp.FirstName, emp.LastName), TimeDone = DateTime.Now};
            }
            catch (Exception ex)
            {
