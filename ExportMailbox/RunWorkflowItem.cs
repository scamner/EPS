...
namespace ItemToRun
{
    public class RunWorkflowItem
    {
        public Utilities.ItemRunResult RunItem(int EmpID, RunPayload RunPayload)
        {
            ...
            String userFolder = String.Format("{0}\\{1}\\{2}\\MailBox_Export", disabledFolderPath, DateTime.Now.Year.ToString(), user.SamAccountName);
            ...
            try
            {
                ...
                Script = Script + String.Format("New-MailboxExportRequest -Mailbox {0} -FilePath \"{1}\\MailboxExport.pst\" -Priority High", emp.Username, userFolder);
                ...
                return new Utilities.ItemRunResult { ResultID = 2, ResultText = String.Format("{0} {1}'s mailbox export was started going to {2}. Exchange Server will continue to process until it is complete.", emp.FirstName, emp.LastName, userFolder), TimeDone = DateTime.Now };
            }
            catch (Exception ex)
            {
